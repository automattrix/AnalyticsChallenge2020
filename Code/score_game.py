...
def yds_allow(start_yd, down, yds_to_go, touchdown, safety, player_safety, def_sideoffield, offense_yards,
    ...
    if proximity_to_play is True:
        ...
        if tfl > 0:
            ...
            pct_away_endzone = pct_away_from_endzone(start_yd=start_yd, tfl_yds=tfl_yds,
...
def score_plays(df, avgs):
    ...
    df['yds_allow_points'] = df[
        ['StartYard', 'Down', 'ToGo', 'FirstDown', 'Touchdown', 'Safety', 'PlayerSafety', 'DefSideOfField',
         'OffensiveYardage',
         'proximity_to_play', 'TackleForLoss', 'tfl_yards', 'offense_neg_yds', 'ForcedFumble', 'RecoveredFumble',
         'Turnover']
    ].apply(
        lambda x: yds_allow(
...
def main():
    ...
    for game in unique_games:
        df_game = df.loc[df['GameID'] == game]

        unique_positions = df_game['TechniqueNameLR'].unique()
        ...
        for position in unique_positions:
            ...
            pdiaa_dict[f"{game}_{position}"] = [pdiaa_avg, pdiaa_sum, pdiaa_count]
    ...
    pdiaa_df.to_csv('./data/04_graph_inputs/pdira_game.csv', index=False)
    print(pdiaa_df)
