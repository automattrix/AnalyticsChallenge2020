...
def yds_allow(start_yd, down, yds_to_go, touchdown, safety, player_safety, def_sideoffield, offense_yards,
              proximity_to_play, tfl, tfl_yds, contributions, interception, pass_break, pressure, completion,
              avgs, f_fumble, rec_fumble, turnover, s_sack, a_sack):
...
def score_plays(df, avgs):
    ...
    df['yds_allow_points'] = df[
        ['StartYard', 'Down', 'ToGo', 'FirstDown', 'Touchdown', 'Safety', 'PlayerSafety', 'DefSideOfField',
         'OffensiveYardage', 'proximity_to_play', 'TackleForLoss', 'tfl_yards', 'offense_neg_yds', 'Interception',
         'PassBreakup', 'Pressure', 'Completion', 'ForcedFumble', 'RecoveredFumble', 'Turnover', 'SoloSack', 'AssistedSack']
    ].apply(
        lambda x: yds_allow(
            ...
            interception=x['Interception'],
            pressure=x['Pressure'],
            completion=x['Completion'],
            pass_break=x['PassBreakup'],
            f_fumble=x['ForcedFumble'],
            rec_fumble=x['RecoveredFumble'],
            turnover=x['Turnover'],
            s_sack=x['SoloSack'],
            a_sack=x['AssistedSack'],
            avgs=avgs),
        axis=1)
...
def main():
    df_avg = pd.read_csv('./data/03_primary/pass_avg_player_contribution.csv')
    df = pd.read_csv('./data/03_primary/pass_processed.csv')
    ...
    for position in unique_positions:
        ...
        # Positional Defensive Pass Impact Above Average
    ...
    pdiaa_df.columns = ['position', 'pdipa_avg', 'pdipa_sum', 'pdipa_count']
    pdiaa_df = pdiaa_df.sort_values(by=['position'], ascending=True)
    pdiaa_df.to_csv('./data/04_graph_inputs/pdipa.csv', index=False)
    print(pdiaa_df)
